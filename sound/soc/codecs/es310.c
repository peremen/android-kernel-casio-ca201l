...
#define FIRMWARE_IMAGE_NUM 10
static unsigned char *fwImage[FIRMWARE_IMAGE_NUM]={
	es310_firmwareimage1,es310_firmwareimage2,es310_firmwareimage3,es310_firmwareimage4,es310_firmwareimage5,
	es310_firmwareimage6,es310_firmwareimage7,es310_firmwareimage8,es310_firmwareimage9,es310_firmwareimage10
};
#endif
...
#ifdef FEATURE_ES310_PATH_UPDATE 
    int amp_cur_path; 
...
static int es310_i2c_read_reg_8(struct i2c_client *i2c,u8 *val)
{
	struct es310_priv *es310 = i2c_get_clientdata(i2c);
	int ret;
	...
	ret = i2c_master_recv(i2c, rxdata, 1);
	if (ret)
		goto out;
...
static int es310_i2c_write_data(struct i2c_client *i2c,unsigned char *txdata, int length)
{
	struct i2c_msg msg[] = {
		{
			.addr = ES310_I2C_ADDRESS,
			.flags = 0,
			.len = length,
			.buf = txdata,
		 },
	...
	if (i2c_transfer(i2c->adapter, msg, 1) < 0) {
		pr_err("es310_i2c_write_data faild \n");
		return -EIO;
	}
    pr_debug("es310_i2c_txdata %d byte ok\n",length);
	return 0;
}
#endif


#ifndef ES310_REQUEST_FIRMWARE
...
#else
    for(count=0;count<total_cmd;count=+2)
	{
	    
	    
    	ret = es310_i2c_write_reg_16(i2c,es310->mode_data[count],es310->mode_data[count+1]);
		pr_debug("es310_i2c_write_ndata reg 0x%x val 0x%x count %d\n",es310->mode_data[count],es310->mode_data[count+1],count);
	}
#endif	
...
#if 0 
static int es310_path_control(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *k, int event)
{
	pr_debug("%s() %x\n", __func__, SND_SOC_DAPM_EVENT_ON(event));

	if (SND_SOC_DAPM_EVENT_ON(event)) {
		if(!strncmp(w->name, "ES310 Amp", 10)) {			
			pr_debug("%s() %x\n", __func__,1);
...
#if 0 
#ifdef FEATURE_ES310_PATH_UPDATE
	SND_SOC_DAPM_SPK("ES310 Amp", es310_path_control), 
#else
...
static int es310_amp_sleep(void)
{
	...
	}while((synAck != ES310_SLEEP_ACK_VAL)&&(count<30));
    #endif
	...
	do{
		...
		msleep(1);
	    ret = es310_i2c_read_reg_32(client,&synAck);
	}while((synAck != ES310_SLEEP_ACK_VAL)&&(count<100));
...
static int es310_amp_set_output(es310_feature_type feature)
{
    struct i2c_client *client = es310_i2c;
    struct es310_priv *es310 = i2c_get_clientdata(client);
    int amp_cur_path = es310->amp_cur_path;
	int yda160_amp_cur_path = es310->yda160_amp_cur_path; 
	...
	switch(feature)
	{
	    ...
	    case ES310_DV_REC:
    	case ES310_WHS_REC:
			pr_debug("%s yda160_amp_cur_path=%d\n", __func__, yda160_amp_cur_path);
			
			if(yda160_amp_cur_path != YDA160_OFF_PATH)
				yda160_amp_set_path(yda160_amp_cur_path, 1);
...
static int es310_amp_off_output(es310_feature_type feature)
{
    struct i2c_client *client = es310_i2c;
    struct es310_priv *es310 = i2c_get_clientdata(client);
    int amp_cur_path = es310->amp_cur_path;
	int yda160_amp_cur_path = YDA160_OFF_PATH; 
	...
	switch(feature)
	{
	    ...
	    case ES310_DV_REC:
    	case ES310_WHS_REC:
			es310->yda160_amp_cur_path = yda160_get_amp_cur_path(); 
			yda160_amp_cur_path = es310->yda160_amp_cur_path; 

			pr_debug("%s yda160_amp_cur_path=%d\n", __func__, yda160_amp_cur_path);
			if(yda160_amp_cur_path != YDA160_OFF_PATH)
				yda160_amp_set_path(yda160_amp_cur_path, 0);
...
static int es310_amp_off_voice_output(void)
{
	struct i2c_client *client = es310_i2c;
    struct es310_priv *es310 = i2c_get_clientdata(client);
    int amp_cur_path = es310->amp_cur_path;
	
    pr_debug("%s start amp_cur_path %d\n", __func__,amp_cur_path);
...
static int es310_firmware_download(void)
{
    ...
    for(ifw=0;ifw<FIRMWARE_IMAGE_NUM;ifw++)
    {
		...
		{
			case 0:
				anc_download = es310_firmwareimage1;
				download_size = sizeof(es310_firmwareimage1);
               break;			
               
			case 1:
				anc_download = es310_firmwareimage2;
				download_size = sizeof(es310_firmwareimage2);
               break;			               

			case 2:
				anc_download = es310_firmwareimage3;
				download_size = sizeof(es310_firmwareimage3);
			   break;			

			case 3:
				anc_download = es310_firmwareimage4;
				download_size = sizeof(es310_firmwareimage4);
			   break;			

			case 4:
				anc_download = es310_firmwareimage5;
				download_size = sizeof(es310_firmwareimage5);
			   break;	

			case 5:
				anc_download = es310_firmwareimage6;
				download_size = sizeof(es310_firmwareimage6);
			   break;			

			case 6:
				anc_download = es310_firmwareimage7;
				download_size = sizeof(es310_firmwareimage7);
			   break;	

			case 7:
				anc_download = es310_firmwareimage8;
				download_size = sizeof(es310_firmwareimage8);

			   break;	
			   
			case 8:
				anc_download = es310_firmwareimage9;
				download_size = sizeof(es310_firmwareimage9);

               break;	
               
			case 9:
				anc_download = es310_firmwareimage10;
				download_size = sizeof(es310_firmwareimage10);
			   break;	
		...
		pr_debug("Downloading %d count %d \n",download_size,ifw);

		mutex_lock(&es310->mutext_lock);

		ret = i2c_master_send(client,anc_download,download_size);
...
static int es310_firmware_download(void)
{
	...
	pr_debug("Downloading %d bytes\n",es310->anc_download_size);

    mutex_lock(&es310->mutext_lock);
	ret = i2c_master_send(client, es310->anc_download,
			      es310->anc_download_size);
	mutex_unlock(&es310->mutext_lock);		      
	if (client < 0) {
		dev_err(&client->dev, "i2c_transfer() failed: %d\n", ret);
	...
	if (ret != es310->anc_download_size) {
		dev_err(&client->dev, "i2c_transfer() failed, %d != %d\n",
			ret, es310->anc_download_size);
		return -EIO;
	}
				      ...
				      const struct i2c_device_id *i2c_id)
{
   ...
   pdata->read_fw_bin = false;
	...
	if(!pdata->read_fw_bin){
	 ...
	 #else
		 ret = es310_i2c_write_reg_16(client,ES310_SYNC_REG,ES310_SYNC_REG_VAL);
		 pr_debug("%s Sync command ret %d\n",__func__,ret);
		 ...
		 do{
			 ...
			 msleep(1);
			 ret = es310_i2c_read_reg_32(client,&synAck);
		 }while((synAck != ES310_SYNC_ACK_VAL)&&(count<100));
	
		 pr_debug("%s sync ack 0x%x ret %d \n",__func__,synAck,ret); 
		 if(ret != 0)
		 {
			
			pr_debug("%s Sync ack fail\n",__func__);
			goto err_fw;
		 }
	 #endif  
