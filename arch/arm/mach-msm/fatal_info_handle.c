...
static int __init fatal_info_handler_probe(struct platform_device *pdev)
{
	...
	#if 0
	reg_buf_addr = res->end + 1;

	reg_buf = ioremap(reg_buf_addr,1024);
	if (reg_buf == NULL) {
		printk(KERN_ERR "fatal_cpu_mmu_regs buffer: failed to map memory\n");
		return -ENOMEM;
	}
	fatal_cpu_mmu_regs_buf = (struct fatal_cpu_mmu_regs *)reg_buf;
	memset(fatal_cpu_mmu_regs_buf, 0, sizeof(struct fatal_cpu_mmu_regs));
	#endif
