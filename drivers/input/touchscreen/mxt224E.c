...
static u8 normal_cfg_config[178][3];
static u8 glove_cfg_config[178][3];
static u8 charger_cfg_config[178][3];
static u8 wireless_cfg_config[178][3];
...
static int mxt_read_block(struct i2c_client *client, u16 addr, u16 length, u8 *value)
{
	...
	if (mxt != NULL) {
		if ((mxt->last_read_addr == addr) && (addr == mxt->msg_proc)) {
...
static int __devinit mxt224E_backup(struct mxt_data *data)
{
	u8 buf = 0x55u;
	int result;
...
static int __devinit mxt224E_init_touch_driver(struct mxt_data *data)
{
	struct object_t *object_table;
	u32 read_crc = 0;
	u32 calc_crc;
	...
	data->device_info.num_nodes = data->device_info.x_size * data->device_info.y_size;
	strcpy(data->device_info.family_name, "mXT224");

	pr_err("[DVE072] family = %d, variant = %d, version = %d, build = %d\n", id[0], id[1], id[2], id[3]);
	pr_err("[DVE072] matrix X size = %d\n", id[4]);
	pr_err("[DVE072] matrix Y size = %d\n", id[5]);
	...
	for (i = 0; i < 5; i++) {
		...
		if (data->fingers[i].status & (RELEASE_MSG_MASK | SUPPRESS_MSG_MASK)) {
			...
			if (touch_mode == TOUCH_GLOVE) {
				...
				} else if ((data->fingers[i].x < 35) || (data->fingers[i].x > 985) || (data->fingers[i].y < 42)) {
					continue;
				} else {
...
static void mxt_ta_worker(struct work_struct *work)
{
	...
	if (error < 0)
		...
		if (debug >= DEBUG_INFO) {
			if (mxt->set_mode_for_ta) {
				pr_debug("[TSP] mxt TA/USB mxt_noise_suppression_config Success!!\n");
			} else {
				pr_debug("[TSP] mxt BATTERY mxt_noise_suppression_config Success!!\n");
			}
		}
	}
...
static void mxt_metal_suppression_off(struct work_struct *work)
{
	...
	pr_debug("[TSP]%s, metal_suppression_chk_flag = %d \n", __func__, metal_suppression_chk_flag);
	disable_irq(mxt->client->irq);
...
static void ts_100ms_tmr_work(struct work_struct *work)
{
	...
	timer_ticks++;
	pr_debug("[TSP] 100ms T %d\n", timer_ticks);
	disable_irq(mxt->client->irq);
	if (facesup_message_flag) {
		pr_debug("[TSP] facesup_message_flag = %d\n", facesup_message_flag);
...
static void cal_maybe_good(struct mxt_data *mxt)
{
	...
	if (mxt_time_point != 0) {
		if ((jiffies_to_msecs(jiffies) - mxt_time_point) >= 300) {
			pr_debug("[TSP] time from calibration start = %d\n", (jiffies_to_msecs(jiffies) - mxt_time_point));
			mxt_time_point = 0;
			cal_check_flag = 0;
			timer_flag = DISABLE;
			timer_ticks = 0;
			...
			mutex_unlock(&mxt->touch_mutex);
			pr_debug("[TSP] Calibration success!! \n");
...
static void mxt_check_wakeup_work(struct mxt_data *mxt)
{
	...
	pr_debug("%s <<<< start mxt_check_wakeup_work >>>>\n", __FUNCTION__);
	...
	pr_debug("<<<< node_cnt tch_ch[%d] atch_ch[%d] finger_cnt[%d]>>>> \n", tch_ch, atch_ch, finger_cnt);
...
static void check_chip_palm(struct mxt_data *mxt)
{
	...
	if ((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00)) {
		...
		pr_debug("[TSP] Flags Counted channels: t:%d a:%d \n", tch_ch, atch_ch);
		data_byte = 0x01;
		...
		if ((tch_ch < 70) || ((tch_ch >= 70) && ((tch_ch - atch_ch) > 25)))
			palm_check_timer_flag = true;
		pr_debug("[TSP] Touch suppression State: %d \n", facesup_message_flag);
	}
}
...
static void check_chip_calibration(struct mxt_data *mxt)
{
	...
	if (cal_check_flag != 1) {
		pr_debug("[TSP] check_chip_calibration just return!! finger_cnt = %d\n", finger_cnt);
		return;
	}
	...
	if ((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00)) {
		...
		pr_debug("[TSP] check_chip_calibration - tch_ch = %d atch_ch = %d finger_cnt = %d\n", tch_ch, atch_ch, finger_cnt);
		...
		if ((touch_mode == TOUCH_GLOVE) && (tch_ch == 0) && (atch_ch == 0))
			tch_ch = 1;
		if ((tch_ch && (tch_ch < 10) && (atch_ch == 0)) && (finger_cnt <= tch_ch)) {
			if ((finger_cnt >= 2) && (tch_ch <= 3)) {
				...
				pr_debug("[TSP] calibrate_timer_wait\n");
			}
...
{
	static u32 pre_x[MXT_MAX_NUM_TOUCHES] = {
	    0,
	};
	static u32 pre_y[MXT_MAX_NUM_TOUCHES] = {
	    0,
	};
	static u32 pre_raw_x[MXT_MAX_NUM_TOUCHES] = {
	    0,
	};
	static u32 pre_raw_y[MXT_MAX_NUM_TOUCHES] = {
	    0,
	};

	static int tcount[MXT_MAX_NUM_TOUCHES] = {
	    0,
	};
	static int pre_raw_distance_x[MXT_MAX_NUM_TOUCHES] = {
	    0,
	};
	static int pre_raw_distance_y[MXT_MAX_NUM_TOUCHES] = {
	    0,
	};
	static int pre_raw_distance[MXT_MAX_NUM_TOUCHES] = {
	    0,
	};
	static int drag_fast_stop[MXT_MAX_NUM_TOUCHES] = {
	    0,
	};
...
static void equalize_coordinate(bool detect, u8 id, u16 *px, u16 *py)
{
	...
	static u16 pre_x[MXT_MAX_NUM_TOUCHES][4] = {
	    {0},
	};
	static u16 pre_y[MXT_MAX_NUM_TOUCHES][4] = {
	    {0},
	};
	int coff[4] = {
	    0,
...
void process_T9_message(u8 *message, struct mxt_data *mxt)
{
	...
	if (status & MXT_MSGB_T9_DETECT) {
		...
		if (facesup_message_flag_T9 == 1) {
			...
			if (debug >= DEBUG_MESSAGES)
				pr_debug("[TSP] Palm(T92) Suppressed !!! \n");
		}
	}
...
void process_T42_message(u8 *message, struct mxt_data *mxt)
{
	...
	if (message[MXT_MSG_T42_STATUS] == MXT_MSGB_T42_TCHSUP) {
		...
		pr_debug("[TSP] Palm(T42) Suppressed !!! \n");
		...
		if (facesup_message_flag) {
			...
			timer_ticks = 0;
			ts_100ms_timer_start(mxt);
			pr_debug("[TSP] Palm(T42) Timer start !!! \n");
		}
	} else {
		pr_debug("[TSP] Palm(T42) Released !!! \n");
...
int process_message(u8 *message, u8 object, struct mxt_data *mxt)
{
	...
	pr_debug("process_message 0: (0x%x) 1:(0x%x) object:(%d)", message[0], message[1], object);

	switch (object) {
	case MXT_PROCG_NOISESUPPRESSION_T48:
		state = message[4];
		if (state == 0x05) {
			pr_err("[TSP] NOISESUPPRESSION_T48 error\n");
		}
		break;
	case MXT_GEN_COMMANDPROCESSOR_T6:
		status = message[1];
		if (status & MXT_MSGB_T6_COMSERR) {
			pr_err("[TSP] maXTouch checksum error\n");
		}
		if (status & MXT_MSGB_T6_CFGERR) {
			pr_err("[TSP] maXTouch configuration error\n");
			mxt_sw_reset(mxt);
			msleep(250);
		}
	...
	case MXT_PROCI_ONETOUCHGESTUREPROCESSOR_T24:
		...
		xpos = message[MXT_MSG_T24_XPOSMSB] * 16 + ((message[MXT_MSG_T24_XYPOSLSB] >> 4) & 0x0F);
		ypos = message[MXT_MSG_T24_YPOSMSB] * 16 + ((message[MXT_MSG_T24_XYPOSLSB] >> 0) & 0x0F);
		xpos >>= 2;
		ypos >>= 2;
	...
	do {
		...
		if ((msg[1] & RELEASE_MSG_MASK) || (msg[1] & SUPPRESS_MSG_MASK)) {
			...
			if (mxt_writecfg_status) {
				if ((data->fingers[0].status == RELEASE_MSG_MASK) || (data->fingers[1].status == RELEASE_MSG_MASK) || (data->fingers[2].status == RELEASE_MSG_MASK) || (data->fingers[3].status == RELEASE_MSG_MASK) || (data->fingers[4].status == RELEASE_MSG_MASK)) {
			...
			data->fingers[report_id].z = msg[5];
			data->fingers[report_id].w = msg[6];
			data->fingers[report_id].x = ((msg[2] << 4) | (msg[4] >> 4)) >> data->x_dropbits;
			data->fingers[report_id].y = ((msg[3] << 4) | (msg[4] & 0xF)) >> data->y_dropbits;
			data->finger_mask |= 1U << report_id;
...
error:
	if (cal_check_flag) {
		...
		pr_debug("[TSP] *** %s report_bfr_num = %d -> report_num = %d calibrate_timer_wait = %d\n",
		...
		if (report_bfr_num && report_num == 0 && calibrate_timer_wait) {
			...
			pr_debug("[TSP] %s finger detect calibrate_timer_wait cancel calibrate_timer_wait = %d  facesup_message_flag = %d\n",
	...
	if (touch_message_flag && (cal_check_flag) && !ts_store_obj_status && !facesup_message_flag) {
		if (timer_flag == DISABLE) {
			mxt = mxt224E_data;
			mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_COMMANDPROCESSOR_T6) + MXT_ADR_T6_DIAGNOSTICS, 0xf3);
			schedule_delayed_work(&mxt224E_data->mxt_reset_dwork, msecs_to_jiffies(60));
		}
	}
...
int write_wireless_config(struct mxt_data *mxt)
{
	...
	return 0;
err_config:
	pr_err("TOUCH write config error\n");
	return -1;
}
				  ...
				  char *buf)
{
	...
	mxt = dev_get_drvdata(dev);
	cradle_chg_int = usb_valid_irq_store();
	count = sprintf(buf, "%d", !cradle_chg_int);
	pr_debug(KERN_INFO "##### cradle_chg_int = %d\n", buf[0]);
		      ...
		      size_t count)
{
	...
	if (count < 3) {
		pr_err(KERN_INFO "set_ap needs to arguments: address pointer "
				 "and data size");
		return -EIO;
	}
	ap = (u16) * ((u16 *)buf);
				  ...
				  struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (debug >= DEBUG_INFO)
		pr_debug("[DIAG/KeyEmulation] %d, emul_key: %d, 0x%X\n", press, emul_key, emul_key);
...
void mxt_normal_config_write(void)
{
	...
	if (mxt_suspend_status == 0) {
		...
		if (ret) {
			pr_err("[TOUCH] %s -> config RAM save fail !!!\n", __func__);
		}
...
static void mxt_resume_delayed_work(struct work_struct *work)
{
	...
	if (!ts_lcd_flag) {
		mutex_unlock(&mxt->touch_mutex);
		pr_debug("do not write i2c cuz LDC29 is off line[%d]\n", __LINE__);
		return;
	}
...
static void mxt_call_recovery_delayed_work(struct work_struct *work)
{
	...
	mxt224E_backup(mxt);
	pr_debug("[tsp]call_recovery_delayed__\n");
	return;
}
...
static void mxt_charging_delayed_work(struct work_struct *work)
{
	...
	if (!ts_lcd_flag) {
		pr_debug("do not write i2c cuz LDC29 is off line return [%d]\n", __LINE__);

		return;
	}

	if (last_charge_mode != charge_mode || last_touch_mode != touch_mode) {
		pr_debug("%s charge_mode = %d \n", __FUNCTION__, charge_mode);
...
static int mxt_load_fw(struct mxt_data *data, const char *fn, size_t count)
{
	...
	if ((mxt_boot_read_mem(client, &boot_status) == 1) && (boot_status & MXT_WAITING_BOOTLOAD_CMD) == MXT_WAITING_BOOTLOAD_CMD) {
			...
			if (read_status_flag == 1) {
				if ((boot_status & MXT_WAITING_BOOTLOAD_CMD) == MXT_WAITING_BOOTLOAD_CMD) {
					if (mxt_unlock_bootloader(client) == 0) {
						mdelay(10);
						pr_debug(KERN_INFO "Unlock OK\n");
					} else {
						pr_debug(KERN_INFO "Unlock fail\n");
					}
					...
					if (0 == frame_size) {
						pr_debug(KERN_INFO "0 == frame_size\n");
						ret = 0;
						goto out;
					}
					next_frame = 1;
					...
					pr_debug(KERN_INFO "CRC Check\n");
				} else if (boot_status == MXT_FRAME_CRC_PASS) {
					if (next_frame == 1) {
						pr_debug(KERN_INFO "CRC Ok\n");
						pos += frame_size;
						next_frame = 0;
						pr_debug(KERN_INFO "Updated %d bytes / %lu bytes\n", pos, fw_size);
					} else {
						pr_debug(KERN_INFO "next_frame != 1\n");
					}
				} else if (boot_status == MXT_FRAME_CRC_FAIL) {
					pr_debug(KERN_INFO "CRC Fail\n");
					crc_error_count++;
				}
		...
		pr_err(KERN_INFO "[TSP] read_boot_state() or (boot_status & 0xC0) == 0xC0) is fail!!!\n");
		ret = 1;
	}
out:
...
static ssize_t show_proximity_status(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	pr_debug("show_proximity_status start\n");

	wake_lock(&touch_wake_lock);
	count = sprintf(buf, "%d", proximity_status);
	pr_debug(KERN_INFO "##### proximity_status = %d\n", buf[0]);
	pr_debug("proximity_status:%d\n", proximity_status);
	wake_unlock(&touch_wake_lock);

	pr_debug("show_proximity_status end\n");
	return (ssize_t)count;
}
...
static ssize_t store_proximity_status(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_debug("store_proximity_status start\n");

	wake_lock(&touch_wake_lock);
	sscanf(buf, "%d", &work_proximity_status);

	pr_debug("Old proximity_status:%d New proximity_status:%d\n", proximity_status, work_proximity_status);
	...
	pr_debug("store_proximity_status end\n");
	return (ssize_t)count;
}
...
static int mxt224E_internal_suspend(struct mxt_data *data)
{
	...
	pr_debug("[TOUCH] touch mode=%d, last touch mode=%d, last charge mode = %d\n", touch_mode, last_touch_mode, last_charge_mode);
				   ...
				   const struct i2c_device_id *id)
{
	...
	data = kzalloc(sizeof(*data) + pdata->max_finger_touches * sizeof(*data->fingers),
		       GFP_KERNEL);
	if (!data)
		return -ENOMEM;
	...
	if (ret) {
		pr_err("[DVE072] : chip initialization failed\n");
		goto err_init_drv;
	}
	...
	for (i = 0; pdata->config[i][0] != RESERVED_T255; i++) {
		...
		if (pdata->config[i][0] == TOUCH_MULTITOUCHSCREEN_T9) {
			if (pdata->config[i][10] & 0x1) {
				data->x_dropbits = (!(pdata->config[i][22] & 0xC)) << 1;
				data->y_dropbits = (!(pdata->config[i][20] & 0xC)) << 1;
			} else {
				data->x_dropbits = (!(pdata->config[i][20] & 0xC)) << 1;
	...
	if (!ts_100s_tmr_workqueue) {
		pr_err(KERN_ERR "unabled to create touch tmr work queue \r\n");
		error = -1;
		goto err_alloc_dev;
	}
	...
	memcpy(normal_cfg_config, normal_booting_config, sizeof(normal_cfg_config));
	memcpy(glove_cfg_config, glove_temp_config, sizeof(glove_cfg_config));
	memcpy(charger_cfg_config, charger_temp_config, sizeof(charger_cfg_config));
	memcpy(wireless_cfg_config, wireless_charger_config, sizeof(wireless_cfg_config));
	normal_cfg_count = 0;
	glove_cfg_count = 0;
	charger_cfg_count = 0;
	wireless_cfg_count = 0;
