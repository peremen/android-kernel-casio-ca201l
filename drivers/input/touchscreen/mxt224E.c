...
static u8 t25_config[] = {SPT_SELFTEST_T25,
				T25_CTRL, 
				T25_CMD,
				T25_SIGLIM_0_UPSIGLIM,
				T25_SIGLIM_0_LOSIGLIM,
				T25_SIGLIM_1_UPSIGLIM,
				T25_SIGLIM_1_LOSIGLIM,
				T25_SIGLIM_2_UPSIGLIM,
				T25_SIGLIM_2_LOSIGLIM};
#endif
...
#if 0
static u8 t24_wireless_config[] = {PROCI_ONETOUCHGESTUREPROCESSOR_T24,
				T18_WIRELESS_CTRL, 
				T18_WIRELESS_COMMAND};


static u8 t25_wireless_config[] = {SPT_SELFTEST_T25,
				T25_WIRELESS_CTRL, 
				T25_WIRELESS_CMD,
				T25_WIRELESS_SIGLIM_0_UPSIGLIM,
				T25_WIRELESS_SIGLIM_0_LOSIGLIM,
				T25_WIRELESS_SIGLIM_1_UPSIGLIM,
				T25_WIRELESS_SIGLIM_1_LOSIGLIM,
				T25_WIRELESS_SIGLIM_2_UPSIGLIM,
				T25_WIRELESS_SIGLIM_2_LOSIGLIM};
#endif
...
#if 0
static int __devinit mxt224E_reset_bootloader(struct mxt_data *data)
{
	u8 buf = 0xA5u;
	return write_mem(data, data->cmd_proc + CMD_RESET_OFFSET, 1, &buf);
}
...
#ifdef ESD_ISSUE
static int __devinit mxt224E_cal(struct mxt_data *data)
{
	u8 buf = 1u;
	return write_mem(data, data->cmd_proc + CMD_CAL_OFFSET, 1, &buf);
}
#endif
...
#if 0
static int mxt_read_diagnostic_object(struct mxt_data *data,u8 type, u16 len, u8 *val)
{
	u16 reg;
	int ret;
	u16 address=0;
...
#if 0
static int mxt224E_reinit_touch_driver(struct mxt_data *data)
{
	struct object_t *object_table;
	u32 read_crc = 0;
	u32 calc_crc;
...
{
	pr_info("reportid:\t0x%x\n", message[0]);
	pr_info("message1:\t0x%x\n", message[1]);
	pr_info("message2:\t0x%x\n", message[2]);
	pr_info("message3:\t0x%x\n", message[3]);
	pr_info("message4:\t0x%x\n", message[4]);
	pr_info("message5:\t0x%x\n", message[5]);
	pr_info("message6:\t0x%x\n", message[6]);
	pr_info("message7:\t0x%x\n", message[7]);
...
#if 0
					else if(data->fingers[i].x < 35) return;
					else if(data->fingers[i].x > 985) return;
					else if(data->fingers[i].y < 42) return;
#else
...
static int suppress_watchdog;

static void suppress_watchdog_func(struct work_struct *work)
{
	if (!suppress_watchdog) {
		mxt224E_cal(the_mxt224E);
		report_input_data(the_mxt224E);
	}
}

static DECLARE_DELAYED_WORK(suppress_watchdog_work, suppress_watchdog_func);
#endif
...
static void cal_maybe_good(struct mxt_data *mxt)
{
	...
	if (mxt_time_point != 0) {
		...
		if ((jiffies_to_msecs(jiffies) - mxt_time_point) >= 300) {
			...
			ret = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_TOUCH_KEYARRAY_T15) + MXT_ADR_T15_TCHTHR, 40);
...
#if 0
static void mxt_calibrate_chip(struct mxt_data *data)
{
	printk("qt602240_calibrate_chip\n");

	if (mxt_cal_check == 0) {
		
		mxt_write_object(data, MXT_GEN_ACQUIRECONFIG_T8, MXT_ADR_T8_ATCHCALST, 0);
		mxt_write_object(data, MXT_GEN_ACQUIRECONFIG_T8, MXT_ADR_T8_ATCHCALSTHR, 0);				
	}

	
	mxt_write_object(data, MXT_GEN_COMMANDPROCESSOR_T6, MXT_ADR_T6_CALIBRATE, 1);
	mxt_cal_check = 1;
}
#endif
...
#if 0
static int mxt_check_abs_time(void)
{
	mxt_time_diff = 0;
...
#if 0
static int mxt224E_resume_touch_speedup(struct mxt_data *data)
{
	int ret;

	ret = mxt_write_byte(data->client, get_object_address(data,MXT_PROCG_NOISESUPPRESSION_T48,0,data->object_table,data->device_info.num_objs) + 0x1B, 5);
	ret = mxt_write_byte(data->client, get_object_address(data,MXT_SPT_CTECONFIG_T46,0,data->object_table,data->device_info.num_objs) + 0x03, 16);
...
static irqreturn_t mxt224E_irq_thread(int irq, void *ptr)
{
...
#ifdef DVE021_TP_LOG
	pr_err("[TSP] *** %s Enter state = %d.\n", __func__, g_config_change_state);
#endif 
	...
	do {
...
#ifdef DVE021_TP_LOG
		printk("%s msg %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
			__func__, msg[0], msg[1], msg[2], msg[3], msg[4], msg[5], msg[6], msg[7], msg[8]);
#endif 
...
#if 0
		if (msg[0] == SUPPRESS_MSG && msg[1] == SUPPRESS_MSG_PRESS) {
		...
		if ((msg[1] & RELEASE_MSG_MASK) || (msg[1] & SUPPRESS_MSG_MASK)) {
			...
			data->fingers[report_id].w = msg[6]; 			
			data->finger_mask |= 1U << report_id;
			data->fingers[report_id].status = SUPPRESS_MSG_MASK;
...
#ifdef DVE021_TP_LOG
	mxt = data;
	mxt_read_byte( mxt->client,  MXT_BASE_ADDR(MXT_GEN_COMMANDPROCESSOR_T6)+ MXT_ADR_T6_CALIBRATE, &cal);

	pr_err("[TSP] *** %s T6 Calibrate = %x\n", __func__, cal);
#endif 
...
#ifdef DVE021_TP_LOG
		pr_err("[TSP] *** %s g_config_change_cal_state = %d \n", __func__, g_config_change_cal_state);
#endif 
...
#ifdef DVE021_TP_LOG
	printk("[TOUCH] %s touch_message_flag = %d cal_check_flag = %d calibrate_timer_flag = %d ts_store_obj_status = %d facesup_message_flag = %d timer_flag = %d\n",
			__func__, touch_message_flag, cal_check_flag, calibrate_timer_flag, ts_store_obj_status, facesup_message_flag, timer_flag);
#endif 
...
static ssize_t store_object(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0) 
	{
		printk("Object type: %u, Offset: %u, Value: %u\n", type, offset, val);
		return ret;
	}
...
#else
static int mxt224E_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct mxt_data *data = i2c_get_clientdata(client);

	return mxt224E_internal_suspend(data);
}

static int mxt224E_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct mxt_data *data = i2c_get_clientdata(client);

	return mxt224E_internal_resume(data);
}
#endif
...
power_on_err2:
	...
	INIT_DELAYED_WORK(&data->firmup_dwork, set_mxt_update_exe);
#endif

#ifdef ESD_ISSUE
	INIT_DELAYED_WORK(&suppress_watchdog_work, suppress_watchdog_func);
	the_mxt224E = data;
#endif
	...
	printk("\n[TOUCH] charge_mode = %d \n",charge_mode);
	if(charge_mode==TOUCH_WALL_CHARGE || charge_mode==TOUCH_USB_CHARGE || charge_mode==TOUCH_CRADLE_CHARGE){
		schedule_delayed_work(&data->charger_dwork, msecs_to_jiffies(1000));
		touch_mode = TOUCH_CHARGE;        
	}
	else if(charge_mode==TOUCH_WIRELESS_CHARGE){
		schedule_delayed_work(&data->wireless_dwork, msecs_to_jiffies(1000));
		touch_mode = TOUCH_WIRELESS;
	}
	else if(charge_mode==TOUCH_NOT_CHARGE){
		schedule_delayed_work(&data->normal_dwork, msecs_to_jiffies(1000));
	}
#endif
