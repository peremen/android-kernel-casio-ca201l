...
static int imx111_act_config(void __user *argp)
{
	CDBG("%s called\n", __func__);
	return (int)msm_actuator_config(&imx111_act_t, argp);
}

static int imx111_i2c_add_driver_table(void)
{
	CDBG("%s called\n", __func__);
...
static int __init imx111_i2c_add_driver(void)
{
	CDBG("%s called\n", __func__);
...
int32_t imx111_actuator_init_table(struct msm_actuator_ctrl_t *a_ctrl)
{
	int32_t rc = 0;
	...
	a_ctrl->step_position_table = kmalloc(sizeof(uint16_t) * (IMX111_TOTAL_STEPS_NEAR_TO_FAR_MAX + 1), GFP_KERNEL);
...
static int32_t imx111_act_probe(void *board_info, void *sdev)
{
	struct msm_actuator_info *info;
	info = (struct msm_actuator_info *)board_info;
	imx111_act_t.eeprom_client = (struct msm_camera_eeprom_client *)info->eeprom_client;
