...
#else
#define MT9E013_TOTAL_STEPS_NEAR_TO_FAR_MAX		37
#endif
...
static int32_t mt9e013_set_params(struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	g_regions[3].code_per_step = ((afcalib_data->af_pos_worst_macro-
		...
		(49 - 38);
#else
	g_regions[0].code_per_step = afcalib_data->af_pos_worst_inf;

#if 1
	g_regions[1].code_per_step = (afcalib_data->af_pos_typ_inf-
		afcalib_data->af_pos_worst_inf)/
		(15 - 1);
	g_regions[2].code_per_step = (afcalib_data->af_pos_typ_macro-
		afcalib_data->af_pos_typ_inf)/
		(38 - 15);
	g_regions[3].code_per_step = (afcalib_data->af_pos_worst_macro-
		afcalib_data->af_pos_typ_macro)/
		(49 - 38);
#else
	g_regions[1].code_per_step = (afcalib_data->af_pos_worst_macro-
		afcalib_data->af_pos_worst_inf)/
		(a_ctrl->set_info.total_steps - 1);
#endif
