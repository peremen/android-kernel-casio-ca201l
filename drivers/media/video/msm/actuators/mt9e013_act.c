...
{
	struct msm_camera_eeprom_client *calib_data = (struct msm_camera_eeprom_client *)a_ctrl->user_data;
	struct sensor_afcalib_data *afcalib_data = (struct sensor_afcalib_data *)(calib_data->data_tbl[0].data);

	g_regions[0].code_per_step = afcalib_data->af_pos_worst_inf << 8;
	g_regions[1].code_per_step = ((afcalib_data->af_pos_typ_inf - afcalib_data->af_pos_worst_inf) << 8) / (15 - 1);
	g_regions[2].code_per_step = ((afcalib_data->af_pos_typ_macro - afcalib_data->af_pos_typ_inf) << 8) / (38 - 15);
	g_regions[3].code_per_step = ((afcalib_data->af_pos_worst_macro - afcalib_data->af_pos_typ_macro) << 8) / (49 - 38);
	return 0;
}
...
static int mt9e013_act_config(void __user *argp)
{
	CDBG("%s called\n", __func__);
	return (int)msm_actuator_config(&mt9e013_act_t, argp);
}

static int mt9e013_i2c_add_driver_table(void)
{
	CDBG("%s called\n", __func__);
...
static int __init mt9e013_i2c_add_driver(void)
{
	CDBG("%s called\n", __func__);
...
static int32_t mt9e013_act_probe(void *board_info, void *sdev)
{
...
static int mt9e013_actuator_debug_init(void)
{
	struct dentry *debugfs_base = debugfs_create_dir("mt9e013_actuator", NULL);
	if (!debugfs_base)
		return -ENOMEM;
