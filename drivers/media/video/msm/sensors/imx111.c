...
#if 0
    printk("imx111: ----- otp_read -----\n");
    for( bank = 0; bank <= OTP_READ_BANK_MAX; bank++)
    {
        printk("bank 0x%02X:",bank);
        for ( read_offset = 0; read_offset < 4; read_offset++)
            printk(" 0x%04X",opt_data_uint16[bank*4 + read_offset]);
        printk("\n");
    }
#endif
...
#ifndef CONFIG_FEATURE_DVE021_DVE058
...
{
	int32_t rc = 0;
	const struct msm_camera_sensor_info *data = s_ctrl->sensordata;
	CDBG("%s: %d\n", __func__, __LINE__);
	msm_sensor_probe_on(&s_ctrl->sensor_i2c_client->client->dev);
	msm_camio_clk_rate_set(MSM_SENSOR_MCLK_30HZ);
...
#endif
{
	...
	s_ctrl->reg_ptr = kzalloc(sizeof(struct regulator *)
				      * data->sensor_platform_info->num_vreg,
				  GFP_KERNEL);
	if (!s_ctrl->reg_ptr) {
			      ...
			      int update_type, int res)
{
	...
	if (update_type == MSM_SENSOR_REG_INIT) {
		...
		if (s_ctrl->curr_res != 0 && res == 0) {
			...
			v4l2_subdev_notify(&s_ctrl->sensor_v4l2_subdev,
					   NOTIFY_PCLK_CHANGE, &s_ctrl->msm_sensor_reg->output_settings[res].op_pixel_clk);
			v4l2_subdev_notify(&s_ctrl->sensor_v4l2_subdev,
					   NOTIFY_ISPIF_STREAM, (void *)ISPIF_STREAM(PIX_0, ISPIF_ON_FRAME_BOUNDARY));
...
#if 0
	get_exif_maker_note->ae_sensor_sv       = 0x0007;   
	get_exif_maker_note->ae_sensor_iso      = 0x0008;   
	get_exif_maker_note->ae_sensor_db       = 0x0009;   
	get_exif_maker_note->awb_rg             = 0x000A;   
	get_exif_maker_note->awb_bg             = 0x000B;   
	get_exif_maker_note->af_focussing       = 0x000C;   
	get_exif_maker_note->af_pos             = 0x000D;   
	get_exif_maker_note->af_horizontal_addr = 0x000E;   
	get_exif_maker_note->af_vertical_addr   = 0x000F;   
	get_exif_maker_note->af_horizontal_size = 0x0010;   
	get_exif_maker_note->af_vertical_size   = 0x0011;
#endif
