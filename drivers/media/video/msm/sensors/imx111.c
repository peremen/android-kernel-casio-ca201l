...
static int32_t imx111_sensor_otp_read(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	for (bank = 0; bank <= OTP_READ_BANK_MAX; bank++) {
		rc = msm_camera_i2c_write( s_ctrl->sensor_i2c_client,
		    OTP_READ_BANK_ADDR, bank, MSM_CAMERA_I2C_BYTE_DATA);
		...
		for (read_offset = 0; read_offset < OTP_READ_BANK_DATA_SIZE; read_offset += sizeof(uint16_t)) {
			rc = msm_camera_i2c_read(
			    s_ctrl->sensor_i2c_client, read_addr,
...
int32_t msm_sensor_imx111_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (cam_vana == NULL) {
		...
		if (regulator_enable(cam_vana)) { CDBG("%s: VREG CAM VANA enable failed\n", __func__);
			rc = -1;
		}
	}
	if (rc < 0) {
...
void imx111_set_dev_addr(struct msm_camera_eeprom_client *eclient, uint16_t *reg_addr)
{
