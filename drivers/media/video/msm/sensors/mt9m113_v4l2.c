...
#else
#define MT9M113_LOG_ERR(fmt, args...) \
	do                            \
	{                             \
	} while (0)
#endif
...
#else
#define MT9M113_LOG_DBG(fmt, args...) \
	do                            \
	{                             \
	} while (0)
#endif
...
static int32_t mt9m113_set_wb(struct msm_sensor_ctrl_t *s_ctrl, int wb_val)
{
	...
	if (setting_data) {
		rc = msm_camera_i2c_write_tbl(s_ctrl->sensor_i2c_client,
		    setting_data, setting_size, MSM_CAMERA_I2C_WORD_DATA);
		if (rc < 0) {
				      ...
				      struct get_exif_param_inf *get_exif_param)
{
	...
	struct reg_access_param_t reg_access_params[] = {
		...
		{MT9M113_ACCESS_REGISTERS, 0x300C, &line_length_DVE047},
	...
	MT9M113_LOG_DBG("line_length_DVE047         = 0x%04X", get_exif_param->line_length_DVE047);
