...
#if 0
static void mt9d115_sub_sensor_reg_check(struct msm_camera_i2c_reg_conf *reg_conf_tbl, uint16_t size)
{
	int i = 0;
	uint16_t val = 0xFFFF;
	...
	for (i = 0; i < size; i++) {
        ...
        if(reg_conf_tbl[i].reg_addr == REG_MT9D115_MCUADD){

            msm_camera_i2c_write(mt9d115_sub_s_ctrl.sensor_i2c_client,
                                REG_MT9D115_MCUADD, reg_conf_tbl[i].reg_data, MSM_CAMERA_I2C_WORD_DATA );
            mdelay(1);
            msm_camera_i2c_read(mt9d115_sub_s_ctrl.sensor_i2c_client,
                                REG_MT9D115_MCUDAT, &val, MSM_CAMERA_I2C_WORD_DATA );
            MT9D115_SUB_LOG_DBG("addr(0x%04X) : set(0x%04X) : now(0x%04X)",reg_conf_tbl[i].reg_data,reg_conf_tbl[i+1].reg_data,val);
            i++;
        }
        else{
            msm_camera_i2c_read(mt9d115_sub_s_ctrl.sensor_i2c_client,
                                reg_conf_tbl[i].reg_addr, &val, MSM_CAMERA_I2C_WORD_DATA );
            MT9D115_SUB_LOG_DBG("addr(0x%04X) : set(0x%04X) : now(0x%04X)",reg_conf_tbl[i].reg_addr,reg_conf_tbl[i].reg_data,val);
        }
...
#if 0
static int32_t mt9d115_sub_reg_AEmeasurementMethod_setting(enum mt9d115_AE_measurement_method setting)
{
	int32_t rc = -EIO;

	MT9D115_SUB_LOG_DBG("mt9d115_sub_reg_AEmeasurementMethod_setting called\n");

	rc = msm_sensor_write_conf_array(mt9d115_sub_s_ctrl.sensor_i2c_client,
									mt9d115_AE_measurement_method_settings, setting);
	if (rc < 0)
		return rc;
...
static int32_t mt9d115_sub_reg_NoiseReduction_setting(enum mt9d115_Noise_Reduction setting)
{
	int32_t rc = -EIO;

	MT9D115_SUB_LOG_DBG("mt9d115_sub_reg_NoiseReduction_setting called\n");

	rc = msm_sensor_write_conf_array(mt9d115_sub_s_ctrl.sensor_i2c_client,
									mt9d115_Noise_Reduction_settings, setting);
	if (rc < 0)
		return rc;
...
static int32_t mt9d115_sub_reg_Sharpness_setting(enum mt9d115_Sharpness_setting setting)
{
	int32_t rc = -EIO;

	MT9D115_SUB_LOG_DBG("mt9d115_sub_reg_Sharpness_setting called\n");

	rc = msm_sensor_write_conf_array(mt9d115_sub_s_ctrl.sensor_i2c_client,
									mt9d115_Sharpness_setting_settings, setting);
	if (rc < 0)
		return rc;
...
#endif
#if 0
static int32_t mt9d115_sub_reg_ZOOM_setting(enum mt9d115_ZOOM_setting setting, int size, enum msm_zoom_camera_mode mode)
{
	msm_camera_i2c_conf_array *setting_array=NULL;
	int32_t rc = -EIO;

	MT9D115_SUB_LOG_DBG("%s setting :%d\n", __func__, setting);
	MT9D115_SUB_LOG_DBG("%s size :%d\n", __func__, size);
	MT9D115_SUB_LOG_DBG("%s mode :%d\n", __func__, mode);

	if(mode == ZOOM_PICTURE){
		switch(size){
		case SENSOR_CAMERA_WXGA_SIZE:
			setting_array = mt9d115_ZOOM_wxga_setting_settings;
			break;

		case SENSOR_CAMERA_VGA_SIZE:
			setting_array = mt9d115_ZOOM_vga_setting_settings;
			break;

		default:
			MT9D115_SUB_LOG_ERR("INVALID");
			break;
		}
	}else if(mode == ZOOM_MOVIE){
		switch(size){
		case SENSOR_CAMERA_VGA_SIZE:
			setting_array = mt9d115_ZOOM_stabilize_off_vga_setting_settings;
			break;

		case SENSOR_CAMERA_QVGA_SIZE:
			setting_array = mt9d115_ZOOM_stabilize_off_qvga_setting_settings;
			break;

		case SENSOR_CAMERA_QCIF_SIZE:
			setting_array = mt9d115_ZOOM_stabilize_off_qcif_setting_settings;
			break;
	...
	}else if(mode == ZOOM_MOVIE_SHAKE){
		switch(size){
		case SENSOR_CAMERA_VGA_SIZE:
			setting_array = mt9d115_ZOOM_stabilize_on_vga_setting_settings;
			break;

		case SENSOR_CAMERA_QVGA_SIZE:
			setting_array = mt9d115_ZOOM_stabilize_on_qvga_setting_settings;
			break;

		case SENSOR_CAMERA_QCIF_SIZE:
			setting_array = mt9d115_ZOOM_stabilize_on_qcif_setting_settings;
			break;
    ...
    if(setting_data){
		...
		if (rc < 0){
			MT9D115_SUB_LOG_ERR("i2c set err setting:%d\n", setting );
			return rc;
		}
		...
		if(rc < 0)
			return rc;
		rc = msm_camera_i2c_write(mt9d115_sub_s_ctrl.sensor_i2c_client, REG_MT9D115_MCUDAT, MT9D115_REFSEQ_DAT, MSM_CAMERA_I2C_WORD_DATA);
		if(rc < 0)
			return rc;
...
static int32_t mt9d115_sub_set_pict_size(struct msm_sensor_ctrl_t *s_ctrl, int pict_size)
{
	...
	case 96: 
		...
		break;
#else
		if((prev_mode == SENSOR_PREVIEW_MODE) || (prev_mode == SENSOR_SNAPSHOT_MODE)){
			MT9D115_SUB_LOG_DBG("SENSOR_CAMERA_VGA_SIZE(V)\n");
			size_setting = MT9D115_SIZE_03;	
		}else if(prev_mode == SENSOR_MOVIE_MODE){
			MT9D115_SUB_LOG_DBG("SENSOR_CAMERA_VGA_SIZE(C)\n");
			size_setting = MT9D115_SIZE_02;	
		}else{
			MT9D115_SUB_LOG_ERR("mt9d115_sub_set_pict_size err mode prev_mode:%d\n", prev_mode);
		}
		break;
#endif
...
#if 0
		rc = msm_sensor_write_conf_array(s_ctrl->sensor_i2c_client,
										mt9d115_SIZE_setting_settings, size_setting);
#endif
		if(rc < 0){
...
#if 0 
		mt9d115_pre_size = pict_size;
		mt9d115_pre_size_width = width;
		mt9d115_pre_size_height = height;

		if( (prev_mode == SENSOR_MOVIE_MODE) && (mt9d115_movie_shake_mode == MOVIE_SHAKE_AUTO) ) {
			mt9d115_zoom.mode = ZOOM_MOVIE_SHAKE;
		} else if( prev_mode == SENSOR_MOVIE_MODE ) {
			mt9d115_zoom.mode = ZOOM_MOVIE;
		} else {
			mt9d115_zoom.mode = ZOOM_PICTURE;
		}

		if( pict_size == SENSOR_CAMERA_VALUE_SIZE ) {
			if( size_setting == MT9D115_SIZE_08 ) {
				mt9d115_zoom.size = SENSOR_CAMERA_HD_SIZE;
			}
			else if( size_setting == MT9D115_SIZE_09 ) {
				mt9d115_zoom.size = SENSOR_CAMERA_VGA_SIZE;
			}
			else if( size_setting == MT9D115_SIZE_10 ) {
				mt9d115_zoom.size = SENSOR_CAMERA_QVGA_SIZE;
			}
			else if( size_setting == MT9D115_SIZE_11 ) {
				mt9d115_zoom.size = SENSOR_CAMERA_QCIF_SIZE;
			}
		} else {
			mt9d115_zoom.size = pict_size;
		}

		
		CAMCMP_LOG_DBG("%s zoom setting :\n", __func__);
		CAMCMP_LOG_DBG("step :%d\n", mt9d115_zoom.step);
		CAMCMP_LOG_DBG("size :%d\n", mt9d115_zoom.size);
		CAMCMP_LOG_DBG("mode :%d\n", mt9d115_zoom.mode);
		rc = mt9d115_sub_reg_ZOOM_setting( mt9d115_zoom.step, mt9d115_zoom.size, mt9d115_zoom.mode);
		if (rc < 0){
			CAMCMP_LOG_DBG("mt9d115_sub_reg_ZOOM_setting err\n");
			return rc;
		}
#endif
	}
...
#if 0
	rc = msm_sensor_write_conf_array(s_ctrl->sensor_i2c_client,
									mt9d115_Scene_settings, setting);
    if(rc < 0)
        return rc;
#endif
...
#if 0
static int32_t mt9d115_sub_set_save_mirror(struct msm_sensor_ctrl_t *s_ctrl, int save_mirror)
{
	int32_t rc = 0;
	enum mt9d115_Position setting;

	MT9D115_SUB_LOG_DBG("start save_mirror = %d\n",save_mirror);

    switch (save_mirror)
    {
        case MSM_CAMERA_SAVE_MIRROR_OFF:
			MT9D115_SUB_LOG_DBG("MSM_CAMERA_SAVE_MIRROR_OFF\n");
			setting = MT9D115_PO_03;
            break;

        case MSM_CAMERA_SAVE_MIRROR_ON:
			MT9D115_SUB_LOG_DBG("MSM_CAMERA_SAVE_MIRROR_ON\n");
			setting = MT9D115_PO_02;
            break;
        ...
        default:
			MT9D115_SUB_LOG_ERR("mt9d115_sub_set_save_mirror default save_mirror=%d\n",save_mirror);
			return 0;
    }

	rc = msm_sensor_write_conf_array(s_ctrl->sensor_i2c_client,
									mt9d115_Position_settings, setting);
	if (rc < 0)
		return rc;
...
#endif
#if 0
static int32_t mt9d115_sub_set_zoom(struct set_zoom_cfg *set_zoom, int size)
{
	int32_t rc = 0;
	...
	if(set_zoom->mode > ZOOM_MOVIE_SHAKE){
		MT9D115_SUB_LOG_ERR("zoom mode failed.\n");
		return -EINVAL;
	}


	MT9D115_SUB_LOG_DBG("zoom = %d, mode = %d, size = %d \n",
			set_zoom->zoom, set_zoom->mode, size);

	if(set_zoom->mode == ZOOM_PICTURE){				
		
		if (size == SENSOR_CAMERA_2MP_SIZE) {
			if ((set_zoom->zoom < CAMCMP_BID_ZOOM_0) || (set_zoom->zoom > CAMCMP_BID_ZOOM_17)) {
				MT9D115_SUB_LOG_ERR("zoom param err\n");
				return -EINVAL;
			}
		} else if (size == SENSOR_CAMERA_WXGA_SIZE) {
			if ((set_zoom->zoom < CAMCMP_BID_ZOOM_0) || (set_zoom->zoom > CAMCMP_BID_ZOOM_26)) {
				MT9D115_SUB_LOG_ERR("zoom param err\n");
				return -EINVAL;
			}
		} else if (size == SENSOR_CAMERA_VGA_SIZE) {
			if ((set_zoom->zoom < CAMCMP_BID_ZOOM_0) || (set_zoom->zoom > CAMCMP_BID_ZOOM_29)) {
				MT9D115_SUB_LOG_ERR("zoom param err\n");
				return -EINVAL;
			}
		}else{
	...
	}else if(set_zoom->mode == ZOOM_MOVIE_SHAKE){		
		if(size == SENSOR_CAMERA_HD_SIZE) {				
			if (set_zoom->zoom != CAMCMP_BID_ZOOM_0) {
				MT9D115_SUB_LOG_ERR("zoom param err\n");
				return -EINVAL;
			}
		} else if (size == SENSOR_CAMERA_VGA_SIZE) {	
			if ((set_zoom->zoom < CAMCMP_BID_ZOOM_0) || (set_zoom->zoom > CAMCMP_BID_ZOOM_23)) {
				MT9D115_SUB_LOG_ERR("zoom param err\n");
				return -EINVAL;
			}
		} else if (size == SENSOR_CAMERA_QVGA_SIZE) {	
			if ((set_zoom->zoom < CAMCMP_BID_ZOOM_0) || (set_zoom->zoom > CAMCMP_BID_ZOOM_29)) {
				MT9D115_SUB_LOG_ERR("zoom param err\n");
				return -EINVAL;
			}
	...
	rc = mt9d115_sub_reg_ZOOM_setting(set_zoom->zoom, size, set_zoom->mode);
	if (rc < 0) {
		MT9D115_SUB_LOG_ERR("mt9d115_reg_ZOOM_setting failed. rc = %d\n", rc);
		return -EFAULT;
	} else {
		mt9d115_sub_zoom.step = set_zoom->zoom;
	}
...
static int32_t mt9d115_sub_set_antishake(struct msm_sensor_ctrl_t *s_ctrl, int antishake)
{
	int32_t rc = 0;

	enum mt9d115_Anti_shaking_setting setting;

	MT9D115_SUB_LOG_DBG("start antishake = %d\n",antishake);

	switch(antishake){
	case 94: 
		MT9D115_SUB_LOG_DBG("ANTI_SHAKE Enable\n");
		setting = MT9D115_AS_00;
		break;

	case 95: 
		MT9D115_SUB_LOG_DBG("ANTI_SHAKE Disable\n");
		setting = MT9D115_AS_01;
		break;
	...
	rc = msm_sensor_write_conf_array(s_ctrl->sensor_i2c_client,
									mt9d115_Anti_shaking_settings, setting);
	if(rc < 0){
		MT9D115_SUB_LOG_ERR("msm_sensor_write_conf_array err antishake:%d rc:%d\n", antishake, rc);
		return -EINVAL;
	}
											...
											struct get_exif_param_inf* get_exif_param)
{
    ...
    struct reg_access_param_t reg_access_params[]={
        ...
        {MT9D115_ACCESS_REGISTERS, REG_MT9D115_LINE_LENGTH_PCK,    &line_length_DVE046        },
    ...
    MT9D115_SUB_LOG_DBG("line_length_DVE046         = 0x%04X",get_exif_param->line_length_DVE046        );
											...
											struct hdr_brightness_t *hdr_brightness)
{
    ...
    MT9D115_SUB_LOG_DBG("line_length_DVE046           = 0x%04X",hdr_brightness->line_length_DVE046          );
