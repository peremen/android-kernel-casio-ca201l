...
int32_t msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	cdata.cfgtype);
		...
		case CFG_SET_FRAME_RATE_MODE:
			...
			rc = s_ctrl->func_tbl->sensor_set_frame_rate_mode( s_ctrl, cdata.cfg.frame_rate_mode);
			break;
		...
		case CFG_SET_CAP_MODE_ENABLE:
			...
			rc = s_ctrl->func_tbl->sensor_set_cap_mode_enable(s_ctrl, cdata.cfg.cap_mode_enable);
			if (rc < 0) {
				pr_err("%s: i2c read/write failed \n", __func__);
				i2c_error_flag = true;
			}
	...
	const struct i2c_device_id *id)
{
	...
	if (s_ctrl->sensor_eeprom_client != NULL) {
		...
		if (info->actuator_info != NULL) {
			info->actuator_info->eeprom_client = (void *)eeprom_client;
		}
