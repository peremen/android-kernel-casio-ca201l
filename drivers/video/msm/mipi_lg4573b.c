...
static int mipi_lg4573b_lcd_on(struct platform_device *pdev)
{
	...
	if ((board_revision == M7SYSTEM_REV_V3A) || (board_revision >= M7SYSTEM_REV_V6A)) {
		...
		if (board_revision == M7SYSTEM_REV_V6A) {
			...
			ret = mipi_dsi_cmds_tx(mfd, &lg4573b_tx_buf, lg4573b_display_bl_brightness_control_cmds_default_case, ARRAY_SIZE(lg4573b_display_bl_brightness_control_cmds_default_case));
		}
...
static void mipi_lg4573b_set_backlight(struct msm_fb_data_type *mfd)
{
...
#if 0
		if(get_backlight_status() == 0 && lcd_status == 2)
		{
			printk("%s, ----------- check : level[%d]\n",__func__,mfd->bl_level);
			return;
		}
#endif
	...
	if ((board_revision == M7SYSTEM_REV_V3A) || (board_revision >= M7SYSTEM_REV_V8A)) {
		...
		if (mfd->bl_level >= BL_WINDOW_SIZE) {
			...
			if (bright_only)
				ret = mipi_dsi_cmds_tx(mfd, &lg4573b_tx_buf, lg4573b_display_bl_brightness_control_only, ARRAY_SIZE(lg4573b_display_bl_brightness_control_only));
			else {
				bright_only = 1;
...
void mipi_lg4573b_disable_display_dstb(struct msm_fb_data_type *mfd)
{
	...
	mipi_dsi_cmds_tx(mfd, &lg4573b_tx_buf, lg4573b_dstb_on_cmds, ARRAY_SIZE(lg4573b_dstb_on_cmds));
	msleep(11);
	mutex_unlock(&mfd->dma->ov_mutex);
...
subsys_initcall_sync(mipi_lg4573b_lcd_init);
#else
