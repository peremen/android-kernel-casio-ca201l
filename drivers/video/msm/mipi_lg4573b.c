...
static int bl_level_table[2][256] = {
    {
	  ...
	  6,   6,   6,   6,  20,  20,  20,  20,  20,  20,  20,  27,  27,  27,  27,  27,
	 27,  27,  34,  34,  34,  34,  34,  34,  34,  34,  42,  42,  44,  44,  44,  44,
	 44,  49,  49,  49,  49,  49,  54,  54,  56,  56,  56,  56,  56,  56,  56,  56,
	 64,  64,  64,  64,  64,  64,  64,  64,  64,  73,  73,  73,  73,  73,  73,  73,
	 73,  73,  73,  73,  84,  84,  84,  84,  84,  84,  84,  84,  84,  84,  94,  94,
	 94,  94,  94,  94,  94,  94,  94,  94, 104, 104, 104, 104, 104, 109, 109, 109,
	109, 109, 109, 115, 115, 115, 115, 115, 115, 115, 115, 115, 115, 125, 125, 125,
	125, 125, 125, 125, 125, 125, 125, 135, 135, 135, 135, 135, 140, 140, 140, 140,
	140, 140, 146, 146, 146, 146, 146, 146, 146, 146, 146, 146, 146, 157, 157, 157,
	157, 157, 157, 157, 157, 157, 157, 157, 168, 168, 168, 168, 168, 173, 173, 173,
	173, 173, 173, 179, 179, 179, 179, 179, 179, 179, 179, 179, 179, 179, 190, 190,
	190, 190, 190, 190, 190, 190, 190, 190, 190, 190, 202, 202, 202, 202, 202, 202,
...
static char config_write_display_brightness_off[2] = {0x51, 0x00};
...
static struct dsi_cmd_desc lg4573b_display_bl_brightness_control_off_cmds[] = {
    {DTYPE_DCS_LWRITE, 1, 0, 0, LG4573B_BL_DELAY, sizeof(config_write_display_brightness_off), config_write_display_brightness_off},
};
...
static void mipi_lg4573b_displayon_delayed_work(struct work_struct *work)
{
	...
	if (mipi_lg4573b_mfd) {
		...
		pr_debug("mipi_lg4573b_displayon_delayed_work execute Display On!!\n");
		lcd_status = 2;
	}
...
static int mipi_lg4573b_lcd_on(struct platform_device *pdev)
{
	...
	pr_debug("[DVE072_LCD]%s LG4573B LCD on Enter\n", __func__);
	...
	if ((board_revision == M7SYSTEM_REV_V3A) || (board_revision >= M7SYSTEM_REV_V8A)) {
		if (lcd_first_init) {
			int gpio24;
			pr_debug("[DVE072_LCD]%s LG4573B LCD first init 1 [%d] \n", __func__, lcd_first_init);

			gpio24 = PM8921_GPIO_PM_TO_SYS(24);

			if (!gpio_get_value_cansleep(gpio24)) {
				pr_debug("[DVE072_LCD]%s LG4573B LCD first init 2 \n", __func__);
	...
	ret = mipi_dsi_cmds_tx(mfd, &lg4573b_tx_buf, lg4573b_power_up_cmds,
	...
	if ((board_revision == M7SYSTEM_REV_V3A) || (board_revision >= M7SYSTEM_REV_V6A)) {
		...
		if (board_revision == M7SYSTEM_REV_V6A) {
			...
			pr_debug("[DVE072_LCD]%s LG4573B LCD first init 3 =========[%d] \n", __func__, lcd_first_init);

			if (0)
				ret = mipi_dsi_cmds_tx(mfd, &lg4573b_tx_buf, lg4573b_display_bl_control_cmds, ARRAY_SIZE(lg4573b_display_bl_control_cmds));
		...
		ret = mipi_dsi_cmds_tx(mfd, &lg4573b_tx_buf, lg4573b_display_bl_control_cmds_1,
				       ARRAY_SIZE(lg4573b_display_bl_control_cmds_1));
	}

	lcd_status = 1;

	ret = mipi_dsi_cmds_tx(mfd, &lg4573b_tx_buf, lg4573b_sleep_off_cmds,
	...
	pr_debug("[DVE072_LCD]%s LG4573B LCD on Exit \n", __func__);
	return 0;
}
...
{
	struct msm_fb_data_type *mfd;
	pr_debug("[DVE072_LCD]%s LG4573B LCD off Enter\n", __func__);
	mfd = platform_get_drvdata(pdev);
	...
	if ((board_revision == M7SYSTEM_REV_V3A) || (board_revision >= M7SYSTEM_REV_V8A)) {
		pr_debug("[DVE072_LCD]%s LG4573B LCD off Enter2 [%p][%d]\n", __func__, mfd, mfd->key);
		if (0)
			mipi_dsi_cmds_tx(mfd, &lg4573b_tx_buf, lg4573b_power_off_cmds, ARRAY_SIZE(lg4573b_power_off_cmds));
		lm3530_lcd_backlight_disable(0x00);
	}

	lcd_status = 0;

	pr_debug("[DVE072_LCD]%s LG4573B LCD off Exit\n", __func__);
	return 0;
}
...
static void mipi_lg4573b_set_backlight(struct msm_fb_data_type *mfd)
{
			...
			if (lcd_status == 1) {
				...
				if (mipi_lg4573b_mfd) {
					...
					mipi_dsi_cmds_tx(mipi_lg4573b_mfd, &lg4573b_tx_buf, lg4573b_lcdm_display_on_cmds,
							 ARRAY_SIZE(lg4573b_lcdm_display_on_cmds));
					mutex_unlock(&mipi_lg4573b_mfd->dma->ov_mutex);
					pr_debug("mipi_lg4573b_displayon_delayed_work execute Display On leesh !!\n");
				}
				mutex_unlock(&g_lcd_mutex);
			}
	...
	pr_debug("[DVE072_LCD]%s get_m7system_board_revision = %x\n", __func__, board_revision);
	...
	if ((board_revision == M7SYSTEM_REV_V3A) || (board_revision >= M7SYSTEM_REV_V8A)) {
		...
		if ((lcd_status >= 1) && (mfd->bl_level == 0)) {
			...
			ret = mipi_dsi_cmds_tx(mfd, &lg4573b_tx_buf, lg4573b_display_bl_brightness_control_off_cmds, ARRAY_SIZE(lg4573b_display_bl_brightness_control_off_cmds));
			lm3530_lcd_backlight_disable(0x00);
		}
	...
	pr_debug("[DVE072_LCD]%s: bl_level_table[mfd->bl_level(=%d)]=%d, config_write_display_brightness[1]= %d\n",
...
ssize_t mipi_lg4573b_lcd_store_onoff(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((board_revision == M7SYSTEM_REV_V3A) || (board_revision >= M7SYSTEM_REV_V8A)) {
		...
		ret = mipi_dsi_cmds_tx(mfd, &lg4573b_tx_buf, lg4573b_display_bl_brightness_control_cmds,
...
static int __devinit mipi_lg4573b_lcd_probe(struct platform_device *pdev)
{
	...
	pr_debug("[DVE072_LCD]%s LG4573B LCD probe\n", __func__);
				 ...
				 u32 channel, u32 panel)
{
	...
	pr_debug("[DVE072_LCD]%s LG4573B LCD device register\n", __func__);
...
{
	pr_debug("[DVE072_LCD]%s LG4573B LCD init\n", __func__);
...
int mipi_lg4573b_user_request_ctrl(struct msmfb_request_parame *data)
{
	...
	default:
		pr_debug("[DVE072_LCD]%s mddi_ta8851_user_request_ctrl\n", __func__);
		break;
	}
