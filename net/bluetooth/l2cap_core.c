...
static struct sk_buff *skb_temp;
static struct l2cap_conn *conn_temp;
static __le16 cid_temp;
static int dir_temp;
...
static void l2cap_queue_acl_data(struct work_struct *worker) {
	...
	for(attempts = 0; attempts < 40; attempts++) {
		msleep(50);
		if(!conn_temp) {
			BT_DBG("conn_temp is NULL");
			return;
		}
		BT_DBG("conn_temp is not NULL");
		sk = l2cap_find_sock_by_fixed_cid_and_dir(cid_temp, conn_temp->src, conn_temp->dst, dir_temp);
		bh_lock_sock(sk);
		if(sk->sk_state == BT_CONNECTED) {
			BT_DBG("attempt number for BT connected state %d", attempts);
			sock_queue_rcv_skb(sk, skb_temp);
			if (sk)
				bh_unlock_sock(sk);
			return;
		}
	...
	if (skb_temp->data[0] != L2CAP_ATT_INDICATE)
		goto not_indicate;
	...
	err_rsp[0] = L2CAP_ATT_CONFIRM;
	memcpy(skb_put(skb_rsp, sizeof(u8)), err_rsp, sizeof(u8));
	hci_send_acl(conn_temp->hcon, NULL, skb_rsp, 0);
	goto free_skb;

not_indicate:
	if (skb_temp->data[0] & L2CAP_ATT_RESPONSE_BIT ||
			skb_temp->data[0] == L2CAP_ATT_CONFIRM)
		goto free_skb;
